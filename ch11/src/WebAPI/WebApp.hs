...
import Web.Scotty.Trans (ScottyT, scottyT, get, text, status, ActionT)
...
loggingMiddleware st app req respond = do
    evalRWST (tact req) st
    app req respond


mainServer :: Port -> IO ()
mainServer port = do
    (settings, initSt) <- initAll
    scottyT port (convertResponse settings initSt) $ do
            -- middleware (loggingMiddleware st)
